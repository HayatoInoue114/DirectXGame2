...
int32_t CreateJoint(const Node& node, const std::optional<int32_t>& parent, std::vector<Joint>& joints) {
	...
	joint.sphere->SetScale({ 0.1f,0.1f,0.1f });
	//joint.sphere->isParent = true;
...
void UpdateSkeleton(Skeleton& skeleton) 
{
	...
	for (Joint& joint : skeleton.joints) {
		...
		if (joint.parent) {
			...
			joint.sphere->SetPosition(Vector3{ joint.skeletonSpaceMatrix.m[3][0],joint.skeletonSpaceMatrix.m[3][1], joint.skeletonSpaceMatrix.m[3][2]});
		}
